...
export default function LeadUploadNode({ data, selected }: NodeProps) {
  return (
    <div className={`relative bg-card border rounded-lg shadow-sm min-w-[200px] transition-all duration-150 ${selected ? 'ring-2 ring-ring ring-offset-1 ring-offset-background border-foreground/20' : 'border-border hover:border-foreground/10'}`}>
      ...
      <div className="px-3 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Lead Upload</span>
      </div>
      <div className="px-3 py-2.5">
        <div className="text-[12px] font-medium">{(data as any).label || 'Import Leads'}</div>
        <div className="text-[10px] text-muted-foreground mt-0.5">CSV · Excel · JSON</div>
      </div>
